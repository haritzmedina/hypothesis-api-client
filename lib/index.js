...
class HypothesisClient {
  ...
  /**
   * Search bulk annotations using search_after
   * @param data
   * @param callback
   */
  searchAnnotationsAfter (data, callback) {
    if (_.isFunction(callback)) {
      let annotations = []
                ...
                setTimeout(() => {
                  ...
                  this.searchBunchAnnotations(d, 0, (err, response) => {
                    if (err) {
                      ...
                      annotations = annotations.concat(response.rows)
                      annotations = this.orderAnnotations(annotations, d.order, d.sort)
                      resolve()
                    }
                  })
                }, 1000)
  ...
  orderAnnotations (annotations, order = 'desc', sort = 'updated') {
