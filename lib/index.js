...
class HypothesisClient {
  ...
  searchAnnotations (data, callback) {
    let annotations = []
    // Function to filter correctly annotations with multiple tags including semicolon. See issue #18
    const filterTags = ({ data, annotations }) => {
      if (data.tag || data.tags) {
        let tags = data.tag || data.tags
        tags = _.isString(tags) ? [tags] : tags
        return annotations.filter((annotation) => {
          return !_.isEmpty(_.intersection(tags, annotation.tags))
        })
      } else {
        return annotations
      }
    ...
    if (!_.isFunction(callback)) {
      console.error('Callback is not defined as a function. Make sure you init it as callback function. E.g.: (err, annotation) => {console.log(annotation)}')
      return
    }
    this.searchBunchAnnotations(data, 0, (err, response) => {
      if (err) {
        ...
        annotations = annotations.concat(response.rows)
        if (response.total === response.rows.length) { // If already retrieved results are all, no required to do more calls
          // Filter correctly annotations with multiple tags including semicolon. See issue #18
          annotations = filterTags({ data, annotations })
          callback(null, annotations)
        } else {
          ...
          Promise.all(promises).then(() => {
            ...
            callback(new Error('Unable to retrieve annotations. One of the multiple calls required to retrieve all annotations is not correctly done after some tries. Check internet connection.'))
          })
        }
      }
    })
