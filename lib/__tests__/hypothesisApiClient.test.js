...
const TEST_GROUP_ID_ONLY_READ = process.env.HYPOTHESIS_TEST_ONLY_READ_GROUP
...
describe('annotations', () => {
  test('fetch', (done) => {
    const hypothesisClient = new HypothesisClient(TOKEN)
    hypothesisClient.fetchAnnotation('zM3SerxzEeu7EMOjequ70w', (err, annotation) => {
      if (err) {
        done.fail(err)
      } else {
        expect(annotation.id).toBe('zM3SerxzEeu7EMOjequ70w')
        expect(annotation.tags[0]).toBe('ThisIsTheOnlyAnnotationWithThisTag')
        done()
      }
    })
  })
  ...
  test('deleteAnnotationParamIsAnAnnotation', (done) => {
    ...
    hypothesisClient.createNewAnnotation(annotationCorpus, (err, annotation) => {
      if (err) {
        // The annotation is not correctly created
        console.error('The annotation is not correctly created. ' + err)
        done.fail(err)
      } else {
  ...
  test('getUserProfile', (done) => {
    ...
    hypothesisClient.getUserProfile((err, profile) => {
      if (err) {
        ...
        expect(profile.userid).toBe('acct:hypothesisApiClient@hypothes.is')
        done()
      }
    })
  })
